...
export function NavItemRoot({ item, isCollapse, open = false, active, onOpen }) {
  const { title, path, icon, info, childrens } = item;

  
  const renderContent = (
    <>
      {icon && <ListItemIconStyle><Iconify icon={icon} sx={{}}/></ListItemIconStyle>}
      <ListItemTextStyle disableTypography primary={title} isCollapse={isCollapse} />
      {!isCollapse && (
        <>
          {info && info}
          {childrens?.length > 0 && <ArrowIcon open={open} />}
        </>
      )}
    </>
  );
...
export function NavItemSub({ item, open = false, active = false, onOpen }) {
  const { title, path, info, childrens } = item;
